...
function handle_add_product($conn, $data) {
    try {
        ...
        // Handle product_type - validate and set default
        $product_type = 'Non-Medicine'; // Default value
        if (isset($data['product_type']) && !empty($data['product_type'])) {
            $product_type = trim($data['product_type']);
            // Validate product_type
            if (!in_array($product_type, ['Medicine', 'Non-Medicine'])) {
                $product_type = 'Non-Medicine';
            }
        }
        ...
        // Prepare insert statement for product (NO quantity or srp - they go to tbl_fifo_stock)
        $stmt = $conn->prepare("
            INSERT INTO tbl_product (
                product_name, category_id, barcode, description, prescription, bulk, product_type,
                expiration, date_added, brand_id, supplier_id,
                location_id, batch_id, status, default_unit
            ) VALUES (
                :product_name, :category_id, :barcode, :description, :prescription, :bulk, :product_type,
                :expiration, :date_added, :brand_id, :supplier_id,
                :location_id, :batch_id, :status, :default_unit
        ...
        $stmt->bindParam(':default_unit', $default_unit);
    ...
    try {
        ...
        $stmt = $conn->prepare("
            SELECT
                ...
                p.location_id,
                l.location_name,
                -- Calculate total quantity from all FIFO batches (sum of all available quantities)
                COALESCE((
                    SELECT SUM(fs_total.available_quantity) 
                    FROM tbl_fifo_stock fs_total 
                    WHERE fs_total.product_id = p.product_id AND fs_total.available_quantity > 0
                ), 0) as total_quantity,
                -- Get oldest batch quantity
                COALESCE((
                    SELECT fs_oldest.available_quantity 
                    FROM tbl_fifo_stock fs_oldest 
                    WHERE fs_oldest.product_id = p.product_id AND fs_oldest.available_quantity > 0
                    ORDER BY fs_oldest.expiration_date ASC, fs_oldest.entry_date ASC 
                    LIMIT 1
                ), 0) as oldest_batch_quantity,
            ...
            LEFT JOIN tbl_location l ON p.location_id = l.location_id
            $whereClause
            AND EXISTS (SELECT 1 FROM tbl_fifo_stock fs WHERE fs.product_id = p.product_id AND fs.available_quantity > 0)
            ORDER BY p.product_name ASC
        ");
    ...
    try {
        ...
        $stmt = $conn->prepare("
            SELECT
                ...
                -- Get quantity from FIFO stock only (no fallback to tbl_product)
            ...
            -- Get total quantities from FIFO stock (real-time)
            LEFT JOIN (
                SELECT
                    product_id,
