...
function getBatchTransferDetailsByProduct($conn, $data) {
    try {
        ...
        // NOTE: tbl_product no longer has srp - use btd.srp from transfer details
        $stmt = $conn->prepare("
            SELECT 
                btd.batch_transfer_id,
                btd.batch_id,
 ...
 * NOTE: tbl_product no longer has quantity or srp columns - these are ONLY in tbl_fifo_stock
 */
function getProductsOldestBatchForTransfer($conn, $data) {
    try {
        ...
        $sql .= "
            GROUP BY p.product_id, fs.batch_id, fs.batch_reference, fs.available_quantity, fs.expiration_date, fs.entry_date, fs.srp
    ...
    try {
        ...
        $sql = "
            SELECT
                ...
                p.barcode,
                COALESCE(first_batch.first_batch_srp, 0) as srp,
                COALESCE(first_batch.first_batch_srp, 0) as unit_price,
                p.location_id,
                l.location_name,
                ...
                COALESCE(first_batch.first_batch_srp, 0) as first_batch_srp
            FROM tbl_product p
            LEFT JOIN tbl_category c ON p.category_id = c.category_id
            ...
            LEFT JOIN (
                SELECT
                    ...
                    ROW_NUMBER() OVER (PARTITION BY fs2.product_id ORDER BY fs2.expiration_date ASC, fs2.entry_date ASC, fs2.fifo_id ASC) as rn
                FROM tbl_fifo_stock fs2
        ...
        $sql .= " GROUP BY p.product_id, p.product_name, c.category_name, p.barcode, p.location_id, l.location_name, b.brand, s.supplier_name ORDER BY earliest_expiry ASC";
...
function force_sync_all_products($conn, $data) {
    try {
        ...
        // NO LONGER NEEDED - tbl_product.quantity column was removed
        // Product quantities are now managed purely through tbl_fifo_stock
        // All quantity operations should go through tbl_fifo_stock directly
        
        echo json_encode([
            "success" => true,
            "message" => "FIFO stock system active - no sync needed",
            "data" => [
                "products_updated" => 0,
                "note" => "Product quantities are now managed through tbl_fifo_stock"
            ]
        ]);
