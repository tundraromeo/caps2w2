...
function handle_update_employee_profile($conn, $data) {
    try {
        ...
        // Check if username is already taken by another employee (if username is provided)
        if (!empty($username)) {
            $checkUsernameStmt = $conn->prepare("SELECT emp_id FROM tbl_employee WHERE username = :username AND emp_id != :emp_id");
            $checkUsernameStmt->bindParam(":username", $username, PDO::PARAM_STR);
            $checkUsernameStmt->bindParam(":emp_id", $emp_id, PDO::PARAM_INT);
            $checkUsernameStmt->execute();
            
            if ($checkUsernameStmt->fetch()) {
                echo json_encode(["success" => false, "message" => "Username already exists for another employee"]);
                return;
            }
        }
        ...
        $updateStmt = $conn->prepare("
            ...
            SET Fname = :fname, Mname = :mname, Lname = :lname, 
                email = :email, username = :username, contact_num = :contact, address = :address,
