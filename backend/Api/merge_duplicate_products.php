        ...
        try {
            $stmt = $conn->prepare("
                SELECT 
                    ...
                    GROUP_CONCAT(COALESCE((SELECT SUM(fs.available_quantity) FROM tbl_fifo_stock fs WHERE fs.product_id = p.product_id), 0) ORDER BY p.product_id) as quantities,
                    GROUP_CONCAT(p.unit_price ORDER BY p.product_id) as prices,
                    SUM(COALESCE((SELECT SUM(fs.available_quantity) FROM tbl_fifo_stock fs WHERE fs.product_id = p.product_id), 0)) as total_quantity,
