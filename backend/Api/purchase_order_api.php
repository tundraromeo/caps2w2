...
try {
    switch ($action) {
        ...
        case 'receive_items':
            receiveItems($conn);
            break;
        case 'get_critical_products_for_po':
            getCriticalProductsForPO($conn);
            break;
...
function createPurchaseOrder($conn) {
    ...
    try {
        ...
        // Insert products - check if item_status and product_id columns exist
        $checkItemStatus = $conn->query("SHOW COLUMNS FROM tbl_purchase_order_dtl LIKE 'item_status'");
        $hasItemStatus = $checkItemStatus->rowCount() > 0;
        
        $checkProductId = $conn->query("SHOW COLUMNS FROM tbl_purchase_order_dtl LIKE 'product_id'");
        $hasProductId = $checkProductId->rowCount() > 0;
        
        // Build INSERT query based on existing columns
        if ($hasProductId && $hasItemStatus) {
            $detailQuery = "INSERT INTO tbl_purchase_order_dtl (purchase_header_id, product_id, quantity, unit_type, product_name, received_qty, missing_qty, item_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        } elseif ($hasProductId) {
            $detailQuery = "INSERT INTO tbl_purchase_order_dtl (purchase_header_id, product_id, quantity, unit_type, product_name, received_qty, missing_qty) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        foreach ($input['products'] as $product) {
            ...
            $productName = $product['productName'] ?? $product['searchTerm'] ?? '';
            
            // Try to get product_id from product_name if not provided
            $productId = null;
            if (isset($product['product_id']) && $product['product_id'] > 0) {
                ...
                $findProductQuery = "SELECT product_id FROM tbl_product WHERE product_name = ? AND (status IS NULL OR status <> 'archived') LIMIT 1";
                $findStmt = $conn->prepare($findProductQuery);
                $findStmt->execute([$productName]);
                $productData = $findStmt->fetch(PDO::FETCH_ASSOC);
                if ($productData) {
            ...
            // Execute with appropriate parameters based on columns
            if ($hasProductId && $hasItemStatus) {
                $detailStmt->execute([
                    $purchaseHeaderId,
...
function getCriticalProductsForPO($conn) {
    try {
        // Get critical products from warehouse based on FIFO stock
        // Uses warehouse low stock threshold (default: 10) matching Warehouse.js logic
        // Priority: Out of Stock (0) > Critical (1-5) > Low Stock (6-10)
        // Based on actual warehouse stock from tbl_fifo_stock - same logic as Warehouse.js
        ...
        $query = "
            SELECT 
                ...
                COALESCE(
                    (SELECT fs2.srp 
                     ...
                     WHERE fs2.product_id = p.product_id 
                     AND fs2.available_quantity > 0
                     ORDER BY fs2.expiration_date ASC 
                     LIMIT 1), 
                    0
                ) as srp,
            ...
            -- Filter based on warehouse FIFO stock (low stock threshold: <= 10)
            -- Matching Warehouse.js logic: isStockLow checks if qty <= lowStockThreshold (default: 10)
            -- Include ALL products with stock <= 10 OR no FIFO stock entries
            AND COALESCE(
                (SELECT SUM(fs.available_quantity) 
                 FROM tbl_fifo_stock fs 
                 WHERE fs.product_id = p.product_id), 
            ...
            ORDER BY 
                current_quantity ASC,  -- Out of stock first, then critical (1-5), then low stock (6-10)
                p.product_name ASC
            LIMIT 50
        ";
        ...
        // If no results, try simpler query without location filter
        if (count($criticalProducts) === 0) {
            // Fallback: Get all products with low stock from FIFO stock regardless of location
            $fallbackQuery = "
                SELECT DISTINCT
                    p.product_id,
                    p.product_name,
            ...
            AND COALESCE(
                ...
                ORDER BY current_quantity ASC, p.product_name ASC
                LIMIT 50
            ";
            
            $fallbackStmt = $conn->prepare($fallbackQuery);
            $fallbackStmt->execute();
            $criticalProducts = $fallbackStmt->fetchAll(PDO::FETCH_ASSOC);
        }
        ...
        foreach ($criticalProducts as $product) {
            $qty = intval($product['current_quantity']);
            if ($qty == 0) {
                $counts['out_of_stock']++;
            } elseif ($qty <= 5) {
                $counts['critical']++;
        ...
        // Debug: Log the results (can be removed later)
        error_log("Critical Products Query Result: " . count($criticalProducts) . " products found");
        if (count($criticalProducts) > 0) {
            error_log("Sample products: " . json_encode(array_slice($criticalProducts, 0, 3)));
        }
        
        echo json_encode([
            'success' => true,
            'data' => $criticalProducts,
            'count' => count($criticalProducts),
            'counts' => $counts,
            'debug_info' => [
                'query_type' => 'main',
                'products_returned' => count($criticalProducts),
                'alert_levels' => $counts
            ]
        ]);
        
    } catch (Exception $e) {
        error_log("Error in getCriticalProductsForPO: " . $e->getMessage());
        echo json_encode([
            'success' => false,
...
function getPreviousOrdersBySupplier($conn) {
    try {
        ...
        // Get unique products that were previously ordered from this supplier
        // Group by product_name to get the most recent quantity and unit_type
        $query = "
            SELECT 
                pod.product_id,
                pod.product_name,
                pod.quantity,
                pod.unit_type,
                MAX(poh.date) as last_ordered_date,
                COUNT(DISTINCT poh.purchase_header_id) as order_count,
                AVG(pod.quantity) as avg_quantity,
                COALESCE(
                    (SELECT fs2.srp 
                     FROM tbl_fifo_stock fs2
                     WHERE fs2.product_id = pod.product_id 
                     AND fs2.available_quantity > 0
                     ORDER BY fs2.expiration_date ASC 
                     LIMIT 1), 
                    0
            ...
            FROM tbl_purchase_order_dtl pod
            INNER JOIN tbl_purchase_order_header poh ON pod.purchase_header_id = poh.purchase_header_id
            WHERE poh.supplier_id = ?
            AND pod.product_name IS NOT NULL
            AND pod.product_name != ''
            GROUP BY pod.product_id, pod.product_name, pod.unit_type
            ORDER BY last_ordered_date DESC, pod.product_name ASC
            LIMIT 50
        ";
        
        $stmt = $conn->prepare($query);
        $stmt->execute([$supplierId]);
        $previousProducts = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        echo json_encode([
            'success' => true,
            'data' => $previousProducts,
            'count' => count($previousProducts)
        ]);
