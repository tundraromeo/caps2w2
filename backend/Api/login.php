...
class LoginManager {
     ...
     * Now includes last_seen timestamp for real-time heartbeat tracking (backward compatible)
     */
    private function logLoginActivity($user, $terminalInfo) {
        ...
        $hasLastSeen = $checkColumn->num_rows > 0;
        
        if ($hasLastSeen) {
            // NEW VERSION: With last_seen column
            $stmt = $this->conn->prepare("
                ...
                VALUES (?, ?, ?, CURTIME(), CURDATE(), ?, ?, ?, ?, 'online', NOW())
            ");
        } else {
