...
function populateMissingBatchDetails($db, $data) {
    try {
        ...
        foreach ($movements as $movement) {
            ...
            $insertSql = "
                INSERT INTO tbl_transfer_batch_details (
                    product_id, batch_id, fifo_id, batch_reference, quantity, 
            ...
            // Only insert if fifo_id exists and is valid
            if ($movement['fifo_id'] !== null && $movement['fifo_id'] !== '') {
                $db->insert($insertSql, [
                    $movement['product_id'],
                    $movement['batch_id'],
                    $movement['fifo_id'],  // Include fifo_id
