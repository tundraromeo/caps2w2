    ...
    try {
        ...
        $stmt = $conn->prepare("
            SELECT
                ...
                COALESCE(oldest_available_batch.quantity, p.quantity) as oldest_batch_quantity,
            ...
            LEFT JOIN (
                SELECT 
                    ...
                    fs.expiration_date,
                    fs.srp,
                    ROW_NUMBER() OVER (PARTITION BY fs.product_id ORDER BY fs.entry_date ASC, fs.batch_reference ASC) as rn
...
function handle_get_product_batches_for_adjustment($conn, $data) {
    try {
        ...
        $stmt = $conn->prepare("
            ...
            WHERE fs.product_id = ? 
                AND fs.quantity > 0
            ORDER BY fs.entry_date ASC, fs.batch_reference ASC
        ");
